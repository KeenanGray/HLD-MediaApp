...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	public static float PluginVersionAsFloat = 1.05f;
...
*/
	...
	private void ActivateContainer_Internal(AccessibleUIGroupRoot container, bool activate, bool readCurrentItem = true)
	{
		...
		else
		{
			...
			if (container.m_PopUp)
			{
				...
				// #UAP: Restore previously active container index instead.
	...
	public static void EnableAccessibility(bool enable, bool readNotification = false)
	{
		...
		if (enable)
		{
			// #UAP: Check if this is the first time that accessibility was turned on ever
...
#if UNITY_EDITOR
		// If this is the Editor, and the left ALT (Left Command on a Mac) key is held, this emulates a two finger tap
		if (Input.GetMouseButtonDown(0) && (Input.GetKey(KeyCode.LeftAlt) == true || Input.GetKey(KeyCode.LeftCommand) == true))
		{
	...
	void UpdateThreeFingerTapDetection()
	{
		...
		if (Input.GetMouseButtonDown(1) && (Input.GetKey(KeyCode.LeftAlt) == true || Input.GetKey(KeyCode.LeftCommand) == true))
		{
	...
	// #UAP: Detect double finger tap for pause/unpause event, one finger swipe right and left
	// #UAP: detect the two finger Z scrub for "Back"
	// #UAP: reading from the top of the page - stoppable
	...
	/// <summary>
	/// Will only trigger if no sound is playing, and is interrupted by user input.
	/// Other skippable sounds will not prevent this from playing.
	/// Use this if the announcement shouldn't block the player from navigating the menus.
	/// It can greatly enhance a blind player's ability to play.
	/// Try to always use this instead of Say, whenever your text doesn't contain vital 
	...
	static public string FormatNumberToCurrentLocale(ulong intNumber)
	{
		...
		if (sIsEuropeanLanguage)
		{
			...
			//Debug.Log("Formatting " + intNumber + " as " + formattedNumber);
		}
		else
	...
	static private void DetectEuropeanLanguage()
	{
		...
		//if (Application.systemLanguage == SystemLanguage.English) sIsEuropeanLanguage = true;
		if (Application.systemLanguage == SystemLanguage.German) sIsEuropeanLanguage = true;
		if (Application.systemLanguage == SystemLanguage.Portuguese) sIsEuropeanLanguage = true;
		if (Application.systemLanguage == SystemLanguage.Spanish) sIsEuropeanLanguage = true;
		if (Application.systemLanguage == SystemLanguage.French) sIsEuropeanLanguage = true;
		if (Application.systemLanguage == SystemLanguage.Italian) sIsEuropeanLanguage = true;
	}
