...
public class UAP_AudioQueue : MonoBehaviour
{
	...
	public void Initialize()
	{
		...
		{
			// Use Google TTS if set up correctly
			if (UAP_AccessibilityManager.GoogleTTSAPIKey.Length == 0)
			{
				// Let the user know that the plugin will be silent if this setup is not corrected
				Debug.LogWarningFormat("[Accessibility] Platform is set to WebGL, but no custom TTS is active and " +
					"no Google Test-To-Speech API key is provided. You need to specify a custom TTS solution or provide an API key. Voice output will not work otherwise.");
			}
			else
			{
